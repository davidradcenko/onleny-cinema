...
export class GenreService {
  ...
  async getAll(searchTerm?: string) {
    ...
    return this.GenreModel.find(option).select('-updateAt -_v').sort({createdAt: 'desc'}).exec()
  }
  ...
  async update(_id: string, dto: CreateGenreDto) {
    ...
    if (!updateDoc) throw new NotFoundException('Genre not fount')
    return updateDoc
  }
  ...
  async delete(id: string) {
    ...
    if (!deleteDoc) throw new NotFoundException('Genre not fount')
    return deleteDoc
  }
}
